...
labels = ['GREEDY_SE_k1','GREEDY_SE_k2','ILP_SE_k1','ILP_SE_k2']
x_ticks = [5,10,15,20]
...
plt.bar(x[1],y[0], width=barwidth, hatch=hatchs[0], label=labels[0], edgecolor=colors[0], fill=True, color='w', zorder=3)
plt.bar(x[1],y[1], width=barwidth, hatch=hatchs[1], label=labels[1], edgecolor=colors[0], fill=True, color='w', zorder=3, bottom=y[0])
plt.bar(x[2],y[2], width=barwidth, hatch=hatchs[0], label=labels[2], edgecolor=colors[1], fill=True, color='w', zorder=3)
plt.bar(x[2],y[3], width=barwidth, hatch=hatchs[1], label=labels[3], edgecolor=colors[1], fill=True, color='w', zorder=3, bottom=y[2])
# plt.bar(x,y, width=barwidth)
plt.ylim([0,1.05])
plt.plot(stacked = True)
#plt.legend()
plt.legend(loc="upper center",ncol = 2 ,prop={'size':15}, framealpha=0)
